pacman::p_load(dplyr, tidyr, survival, survminer, ggplot2, ggthemr, broom)
...
for_plot <-
  rbind(
    ...
    mutate(tag = "3"),
    tidy(
      coxph(Surv(time, status) ~ percentile + gender + site, data = filter(survival_data, max_rx == 4)),
      exponentiate = TRUE, conf.int = TRUE) %>%
    mutate(tag = "4")) %>%
  filter(!term == "percentileelse") %>%
  mutate(
    term = case_when(
...
p1 <-
  ggplot(for_plot, aes(tag, estimate, color = term)) +
