...
female_txt <-
	sprintf(
		"%d (%.1f%%)",
		sum(wd$gender == "Female"),
		100 * mean(wd$gender == "Female"))

male_txt <-
	sprintf(
		"%d (%.1f%%)",
		sum(wd$gender == "Male"),
		100 * mean(wd$gender == "Male"))
...
fh_yes_txt <-
	sprintf(
		"%d (%.1f%%)",
		sum(wd$any_hist == 1),
		100 * mean(wd$any_hist == 1))

fh_no_txt <-
	sprintf(
		"%d (%.1f%%)",
		sum(wd$any_hist == 0),
		100 * mean(wd$any_hist == 0))
...
age0_txt <-
	sprintf(
		"%.1f (%.1f–%.1f)",
		median(wd$age_W0),
		quantile(wd$age_W0, .25),
		quantile(wd$age_W0, .75))

age1_txt <-
	sprintf(
		"%.1f (%.1f–%.1f)",
		median(wd$age_W1),
		quantile(wd$age_W1, .25),
		quantile(wd$age_W1, .75))

age2_txt <-
	sprintf(
		"%.1f (%.1f–%.1f)",
		median(wd$age_W2),
		quantile(wd$age_W2, .25),
		quantile(wd$age_W2, .75))

age3_txt <-
	sprintf(
		"%.1f (%.1f–%.1f)",
		median(wd$age_W3),
		quantile(wd$age_W3, .25),
		quantile(wd$age_W3, .75))
