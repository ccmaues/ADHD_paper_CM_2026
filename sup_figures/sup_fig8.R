...
all_w0 <- data.frame(
	R2 = c(
  	...
  	PseudoR2(glm(W0 ~ PRS + PC1 + PC2 + PC3 + PC4 + age_W0 + age_W1 + age_W2, family = "binomial", data = all), which = "Nagelkerke")),
		var = c("PRS", "PC1", "PC2", "PC3", "PC4", "age_W0", "age_W1", "age_W2", "Full model")) %>%
	mutate(var = factor(var, levels = c("PRS", "PC1", "PC2", "PC3", "PC4", "age_W0", "age_W1", "age_W2", "Full model")))
...
all_w1 <- data.frame(
  	...
  	PseudoR2(glm(W1 ~ PRS + PC1 + PC2 + PC3 + PC4 + age_W0 + age_W1 + age_W2, family = "binomial", data = all), which = "Nagelkerke")),
		var = c("PRS", "PC1", "PC2", "PC3", "PC4", "age_W0", "age_W1", "age_W2", "Full model")) %>%
	mutate(var = factor(var, levels = c("PRS", "PC1", "PC2", "PC3", "PC4", "age_W0", "age_W1", "age_W2", "Full model")))
...
all_w2 <-
	data.frame(
  	...
  	PseudoR2(glm(W2 ~ PRS + PC1 + PC2 + PC3 + PC4 + age_W0 + age_W1 + age_W2, family = "binomial", data = all), which = "Nagelkerke")),
		var = c("PRS", "PC1", "PC2", "PC3", "PC4", "age_W0", "age_W1", "age_W2", "Full model")) %>%
	mutate(var = factor(var, levels = c("PRS", "PC1", "PC2", "PC3", "PC4", "age_W0", "age_W1", "age_W2", "Full model")))
...
p1 <-
	ggplot(all_w0, aes(var, R2, fill = var)) +
		...
		geom_hline(aes(yintercept = 0.02723204), linetype = "dashed", color = "red") +
		scale_y_continuous(limits = c(0, 0.055), n.breaks = 5, expand = expansion(mult = c(0.05, 0.15))) +
		labs(y = "", x = "") +
		theme_publish() +
		theme(
...
p2 <-
	ggplot(all_w1, aes(var, R2, fill = var)) +
      ...
      scale_fill_grey(start = 0.2, end = 0.8) +
	  geom_hline(aes(yintercept =  0.01037148), linetype = "dashed", color = "red") +
	  scale_y_continuous(limits = c(0, 0.055), n.breaks = 5, expand = expansion(mult = c(0.05, 0.15))) +
...
p3 <-
    ...
    scale_y_continuous(limits = c(0, 0.055), n.breaks = 5, expand = expansion(mult = c(0.05, 0.15))) +
    scale_fill_grey(start = 0.2, end = 0.8) +
    geom_hline(aes(yintercept =  0.01254808), linetype = "dashed", color = "red") +
    labs(y = "", x = "") +
    theme_publish() +
    theme(
        legend.position = "none",
...
ggsave("fig8_sup.png",final, device = "png", width = 200, height = 300, units = "mm", dpi = 300, bg = "white")